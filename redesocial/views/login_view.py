...
def login():
    ...
    print("Root usado no botão:", entry_pass_frame.winfo_toplevel())
    print("Root que carrega imagem:", root)

    eye_button = tk.Button(entry_pass_frame, image=None, command=toggle_password_visibility, bd=0, highlightthickness=0, bg=colors["bg_entry"], activebackground=colors["bg_entry"]); eye_button.pack(side="right", padx=(0,10))
...
#
    ...
    if img_entrar:
        tk.Button(main_frame, image=None, command=lambda:loginController(root,entry_user,entry_pass), bd=0, highlightthickness=0, activebackground=colors["bg_main"]).pack(pady=(40, 10), padx=40, fill="x")
    else:
